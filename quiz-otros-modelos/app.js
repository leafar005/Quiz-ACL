  ...
  window.buildCategoryGrid = function buildCategoryGrid() {
    ...
    CATEGORIES.forEach(cat => {
      ...
      if (format === 'flashcards') {
         let cards = (window.FlashcardsData && window.FlashcardsData['otros-modelos']) ? window.FlashcardsData['otros-modelos'] : [];
      ...
      btn.addEventListener('click', () => {
        ...
        const format = activeFormatBtn ? activeFormatBtn.dataset.format : 'quiz';
        if (format === 'flashcards') {
          window.location.href = `../shared/flashcards.html?topic=otros-modelos&category=${encodeURIComponent(cat.id)}&catName=${encodeURIComponent(cat.name)}`;
        } else {
