  ...
  window.buildCategoryGrid = function buildCategoryGrid() {
    ...
    CATEGORIES.forEach(cat => {
      ...
      if (format === 'flashcards') {
         let cards = (window.FlashcardsData && window.FlashcardsData['iso9000']) ? window.FlashcardsData['iso9000'] : [];
      ...
      btn.addEventListener('click', () => {
        ...
        const format = activeFormatBtn ? activeFormatBtn.dataset.format : 'quiz';
        if (format === 'flashcards') {
          window.location.href = `../shared/flashcards.html?topic=iso9000&category=${encodeURIComponent(cat.id)}&catName=${encodeURIComponent(cat.name)}`;
        } else {
