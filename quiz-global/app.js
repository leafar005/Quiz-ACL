  ...
  window.buildCategoryGrid = function buildCategoryGrid() {
    ...
    CATEGORIES.forEach(cat => {
      ...
      if (format === 'flashcards') {
         let cards = window.FlashcardsData ? Object.values(window.FlashcardsData).flat() : [];
      ...
      btn.addEventListener('click', () => {
        ...
        const format = activeFormatBtn ? activeFormatBtn.dataset.format : 'quiz';
        if (format === 'flashcards') {
          window.location.href = `../shared/flashcards.html?topic=&category=${encodeURIComponent(cat.id)}&catName=${encodeURIComponent(cat.name)}`;
        } else {
